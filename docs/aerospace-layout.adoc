...
* `tabbed` - i3's tabbed layout. Stacks all windows of the container and draws a tab bar above them.
  Orientation doesn't apply. Requires `glass.tabs.enabled` to draw the bar
...
* Toggle between `tiles` and i3's `tabbed` layout: +
`aerospace layout tiles tabbed`
